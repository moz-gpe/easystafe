...
- Carregar e processar múltiplas pastas de extractos e-SISTAFE em
  sequência, compilando os resultados automaticamente;
- Limpar, padronizar e filtrar os dados por UGB do sector;
- Desduplicar os valores por grupos CED através de subtracção
  hierárquica;
- Enriquecer o dataset com metadados descritivos (por exemplo,
  província, distrito, nível funcional, tipo de programa);
...
comparação com os dias ou semanas necessárias para realizar o mesmo
trabalho manualmente em Excel.
...
O código encadeado depende dos seguintes pacotes R. Execute o código
abaixo na consola do RStudio uma única vez para os instalar todos:
...
Os extractos do e-SISTAFE devem estar guardados em pastas com a seguinte
convenção de nomenclatura `YYYYMM` (por exemplo, `Data/202602/`). Esta
nomenclatura das pastas é obrigatório pois o pacote lê o nome da pasta
para derivar automaticamente as colunas `ano` e `mes` em cada linha do
dataset. Se uma pasta não seguir este formato, essas colunas serão
...
lista pronta a ser utilizada no código encadeado. A função valida a
presença das três folhas antes de tentar ler qualquer dado. Se alguma
estiver ausente, é emitido um erro claro com o nome da folha em falta.
...
Esta é a etapa central do código encadeado. A função
...
Para consultar a documentação completa de todos os argumentos
disponíveis, execute
[`?processar_extracto_esistafe`](https://moz-gpe.github.io/easystafe/reference/processar_extracto_esistafe.md)
na consola do R.
...
junta metadados descritivos ao dataset.
...
armazenamento eficiente) e Excel (para partilha e revisão manual).
...
presentes nos dados. Por exemplo:
...
base nos dados presentes no dataset.

------------------------------------------------------------------------

## Resumo: Código Encadeado Completo

O código encadeado completo para processar oito meses de extractos do
e-SISTAFE cabe em menos de 50 linhas de código:
