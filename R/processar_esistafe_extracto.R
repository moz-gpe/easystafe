...
#'
#' O dataframe final contém um conjunto de métricas orçamentais padronizadas do
#' e-SISTAFE, incluindo:
#'
#' * **Dotação Inicial (dotacao_inicial)** — Alocação inicial aprovada no Orçamento do Estado.
#' * **Dotação Revista (dotacao_revista)** — Alocação revista formalmente pelo Parlamento.
#' * **Dotação Actualizada – DA (dotacao_actualizada)** —
#'   Atualizações efetuadas pelo MEF dentro dos limites legais, sem revisão parlamentar.
#' * **Dotação Disponível (dotacao_disponivel)** — Liquidez efetivamente disponibilizada.
#' * **Dotação Cabimentada – DC (dotacao_cabimentada)** — Montante cabimentado/comprometido.
#' * **Adiantamento de Fundos – AF (ad_fundos)** —
#'   Montantes adiantados antes da execução; inclui, por exemplo, o ADE para escolas.
#' * **Despesa Paga Via Directa – DP (despesa_paga_via_directa)** —
#'   Pagamentos feitos diretamente ao fornecedor via e-SISTAFE.
#' * **Adiantamento de Fundos + Despesa Paga VD – AFDP (ad_fundos_mais_dpvd)** —
#'   Soma dos adiantamentos e das despesas pagas via direta.
#' * **Adiantamentos de Fundos Liquidados – LAF (liq_ad_fundos)** —
#'   Adiantamentos para os quais o governo já recebeu e aceitou a fatura (aguardando pagamento).
#' * **Despesa Liquidada Via Directa – LVD (despesa_liquidada_via_directa)** —
#'   Despesas liquidadas diretamente no e-SISTAFE, ou seja, com documento de liquidação emitido.
#' * **Liq. Adiantamento de Fundos + Via Directa – LAFVD (liq_ad_fundos_via_directa_lafvd)** —
#'   Total de liquidações combinando adiantamentos liquidados e despesas liquidadas via direta.
#'
#' Estas variáveis refletem todo o ciclo orçamental: dotação, disponibilização de fundos,
